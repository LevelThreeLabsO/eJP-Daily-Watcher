...
class SlackClient:
    ...
    eJP posts two streams into two channels, so this takes the NAME of the environment
    variable holding that channel's webhook (SLACK_GIFTS, SLACK_WWW) rather than a bare
    URL. A literal URL still works — anything containing "://" is used as-is.
    """

    def __init__(self, env_or_url: str | None = None):
        if env_or_url and "://" in env_or_url:
            raw = env_or_url
        elif env_or_url:
            raw = os.environ.get(env_or_url, "")
        else:
            raw = os.environ.get("SLACK_WEBHOOK_URL", "")
        self.source = env_or_url or "SLACK_WEBHOOK_URL"
    ...
    def post(self, text: str) -> None:
        ...
        if not self.webhook_url:
            raise DeliveryError(f"{self.source} is not set")
