...
# Two streams, each its own channel, so the per-line category label the Circuit digest
# carries is redundant here — every line in a message is already the same kind.
CATEGORY_ORDER = ["major_gift", "watching"]
CATEGORY_LABELS = {
...
def format_line(item: Item, now: datetime) -> str:
    ...
    label = ""  # redundant: one stream per message
...
def build(items: list[Item], now: datetime, max_items: int = MAX_ITEMS, heading: str = "",
    ...
    body = text
    if heading:
        # One heading per message. The channel already separates the two streams, but a
        # heading makes a screenshot of a single message self-explanatory.
        body = f"*{heading}*\n{body}"
